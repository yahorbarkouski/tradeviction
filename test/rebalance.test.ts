...
import { rebalanceAction } from "@/app/actions";
import { changeKind, parseBookChanges, planChanges, type BookChange, type Held } from "@/lib/book";
import { countDeployed, getActivePosition, listEventsForStartup, movesLeft } from "@/lib/db/queries";
...
describe("planning a commit", () => {
  ...
  it("names each change the way the Book records it", () => {
    ...
    expect(changeKind(null, { ...base, from: null, direction: "long", conviction: 0, note: "", close: true })).toBeNull();
  });
  ...
  it("parses only well-formed change lists", () => {
    ...
    expect(parseBookChanges(JSON.stringify(Array.from({ length: 51 }, (_, i) => ({ ...one, startupId: `s${i}` })))).ok).toBe(
      false,
    );
  });
});
